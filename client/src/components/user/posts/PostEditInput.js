...
const PostEditInput = ({ showModal, setShowModal, post }) => {
  ...
  const editPostHND = bindActionCreators(editPost, useDispatch());

  // send post data
  const submitPost = async () => {
    setloadingstate(true);

    if (await editPostHND({ text }, post._id)) {
        ...
        <Modal.Body>
          <div className="space-y-6 px-0 sm:px-6 pb-4 sm:pb-6 lg:px-8 xl:pb-8">
            ...
            <div className="w-full">
              <button
                className="bg-indigo-700 py-2 px-4 text-white rounded-md"
                onClick={submitPost}
              >
